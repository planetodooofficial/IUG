...
class select_interpreter_line(osv.osv):
    ...
    _columns = {
        ...
        'company_id': fields.related('event_id','company_id', type="many2one", relation="res.company", store=True, string="Company" ,
                 readonly=True,),
        'select_interpreter': fields.boolean('Offer Job'),
    }
    _defaults={
        'voicemail_msg':'',
        }

    def leave_voicemail_without_warning(self, cr, uid, ids, context=None):
            ...
            if select_line.interpreter_id:
                if select_line.state == 'voicemailsent' and select_line.interpreter_id.id == cur_obj.interpreter_id.id:
                    raise osv.except_osv(_('Warning!'),
                                         _('Selected interpreter is already present in the Job offered list!'))
        ...
        if not cur_obj.interpreter_id.is_agency:
            history_ids2 = history_obj.search(cr, SUPERUSER_ID, [('name', '=', cur_obj.interpreter_id.id),
                                                                 ('state', 'in', ('confirm', 'allocated')),
                                                                 ('company_id', '=', event.company_id.id),
                                                                 ('cancel_date', '=', False),
                                                                 ('event_date', '=', (datetime.datetime.strptime(str(
                                                                     datetime.datetime.strptime(str(event.event_start),
        ...
        voicemail_msg = "Voicemail Message left for " + str(cur_obj.interpreter_id.name) + "for Event " + str(
            event.name) + \
                        " on the date " + str(from_dt.strftime('%Y-%m-%d'))
        self.pool.get('interpreter.history').create(cr, uid,
        ...
        if cur_obj.interpreter_id.user_id and cur_obj.interpreter_id.user_id != SUPERUSER_ID:
            if cur_obj.interpreter_id.user_id and event.state in ['draft', 'scheduled']:
                self.pool.get('event').write(cr, uid, [event.id],
                                             {'event_follower_ids': [(4, cur_obj.interpreter_id.user_id.id)]})

    def raise_warning_multi_interpreter(self, cr, uid, ids, context=None):
        if not ids:
            return False
        ir_model_data = self.pool.get('ir.model.data')
        warning_obj = self.pool.get('warning')
    ...
    def leave_voicemail(self , cr ,uid , ids , context= None):
        ...
        self.leave_voicemail_without_warning(cr, uid, ids, context=context)
	if context.get('res'):
            res = context.get('res')
        ir_model_data = self.pool.get('ir.model.data')
        warning_obj = self.pool.get('warning')
...
#            print "user_ids......",user_ids
    ...
    def job_offer_multiple_interpreter(self, cr, uid, ids, context=None):
        select_interpreter_obj = self.pool.get('select.interpreter.line')
        selected_interp = []
        for event in self.browse(cr, uid, ids):
            for select_inter in event.interpreter_ids:
                if select_inter.state == 'draft' and select_inter.select_interpreter == True:
                    selected_interp.append(select_inter.id)
        for inter_id in selected_interp:
            select_interpreter_obj.leave_voicemail_without_warning(cr, uid, [inter_id], context=context)
        warn_multiple_interpreter = select_interpreter_obj.raise_warning_multi_interpreter(cr, uid, selected_interp,context=context)
...
#                history_ids2.remove(event.id)
	...
	context.update({'res':res})
        voicemail_result = select_obj.leave_voicemail(cr, uid, [int_line_id], context=context)
            ...
            if cur_obj.event_type == 'lang_trans':
                ...
                for interpreter in cur_obj.assigned_interpreters:
                    task_work_vals={
                        ...
                        'event_start_hr': cur_obj.event_start_hr or '00',
                        'event_start_min': cur_obj.event_start_min or '00',
                        'event_end_hr': cur_obj.event_end_hr or '00',
                        'event_end_min': cur_obj.event_end_min or '00',
    ...
    def create_supp_inv(self, cr, uid, ids, context=None):
        ...
        for cur_obj in self.browse(cr, uid, ids):
            ...
            inv_data = {
                ...
                'payment_term': cur_obj.partner_id.property_supplier_payment_term and cur_obj.partner_id.property_supplier_payment_term.id or False,
                'company_id': cur_obj.company_id and cur_obj.company_id.id or False,
