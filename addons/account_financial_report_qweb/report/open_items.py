...
WITH
    accounts_partners AS
        (
            SELECT
                ...
                COALESCE(
                    CASE
                        WHEN
                            NULLIF(pp.name, '') IS NOT NULL
                            AND NULLIF(pp.ref, '') IS NOT NULL
                        THEN pp.name || ' (' || pp.ref || ')'
                        ELSE pp.name
                    END,
                    '""" + _('') + """'
                ) AS parent_partner_name   
            FROM
        ...
        query_inject_partner += """
            LEFT JOIN
                res_partner p ON ml.partner_id = p.id left join res_partner pp on p.parent_cust_id=pp.id
            WHERE
                ra.report_id = %s
                        """
        if self.filter_partner_ids:
        ...
        query_inject_partner += """
            GROUP BY
                ...
                a.id,
                p.id,
                at.include_initial_balance,pp.name,pp.ref
        )
INSERT INTO
