...
class report_xls(report_sxw):
    ...
    def create(self, cr, uid, ids, data, context=None):
        ...
        report_ids = report_obj.search([('report_name', '=', self.name[7:])],limit=1)
        if report_ids:
            self.title = report_ids.name
            if report_ids.report_type == 'xls':
                return self.create_source_xls(ids, data)
    ...
    def create_source_xls(self, ids, data):
        ...
        if tz_name:
            try:
                ...
                _logger.debug("failed to compute context/client-specific timestamp, "
                              "using the UTC value",
                              exc_info=True)
        report_date = utc_timestamp.strftime(DEFAULT_SERVER_DATETIME_FORMAT)
        self.xls_footers = {
