...
class document_file(models.Model):
    ...
    user_id=fields.Many2one('res.users', 'Owner', index=True, default=lambda self: self.env.user)
    parent_id=fields.Many2one('document.directory', 'Directory', index=True, change_default=1)
    ...
    def check(self, mode, values=None):
        ...
        super(document_file, self).check(mode,values=values)
        
        if self.ids:
            self.env['ir.model.access'].check('document.directory', mode)

            # use SQL to avoid recursive loop on read
            self._cr.execute('SELECT DISTINCT parent_id from ir_attachment WHERE id in %s AND parent_id is not NULL', (tuple(self.ids),))
            records= self.env['document.directory'].browse([parent_id for (parent_id,) in self._cr.fetchall()]).exists()
            records.check_access_rule(mode)

    @api.model
    def search(self,args, offset=0, limit=None, order=None,count=False):
        # Grab ids, bypassing 'count'
        ids = super(document_file, self).search(args, offset=offset, limit=limit, order=order,count=False)
        if not ids:
            return 0 if count else self.env['ir.attachment']
        ...
        self._cr.execute('SELECT id, parent_id from ir_attachment WHERE id in %s', (tuple([v.id for v in ids]),))
        # cont a dict of parent -> attach
        parents = {}
        for attach_id, attach_parent in self._cr.fetchall():
            parents.setdefault(attach_parent, []).append(attach_id)
        parent_ids = parents.keys()

        # filter parents
        visible_parent_ids = self.env['document.directory'].search([('id', 'in', list(parent_ids))])
    ...
    def create(self,vals):
        vals['parent_id'] = self._context.get('parent_id', False) or vals.get('parent_id', False)
        # take partner from uid
        if vals.get('res_id', False) and vals.get('res_model', False) and not vals.get('partner_id', False):
            vals['partner_id'] = self.__get_partner_id(vals['res_model'], vals['res_id'])
        if vals.get('datas', False):
            vals['file_type'], vals['index_content'] = self._index(vals['datas'].decode('base64'), vals.get('datas_fname', False),None)
        return super(document_file, self).create(vals)

    @api.multi
    def write(self,vals):
        if vals.get('datas', False):
            vals['file_type'], vals['index_content'] = self._index(vals['datas'].decode('base64'), vals.get('datas_fname', False),None)
        return super(document_file, self).write(vals)
    ...
    @api.model
    def __get_partner_id(self,res_model, res_id):
        ...
        fields = obj_model.fields_get()
#        print 'jfgawgyfv',fields
        if obj_model._name == 'res.partner':
            return res_id
        elif 'partner_id' in fields and fields['partner_id']['relation'] == 'res.partner':
            bro = obj_model.browse(res_id)
...
class document_directory(models.Model):
    ...
    user_id=fields.Many2one('res.users', 'Owner',default=lambda self: self.env.uid)
    group_ids= fields.Many2many('res.groups', 'document_directory_group_rel', 'item_id', 'group_id', 'Groups')
    parent_id=fields.Many2one('document.directory', 'Parent Directory', index=True, change_default=True)
    ...
    type=fields.Selection([ ('directory','Static Directory'), ('ressource','Folders per resource'), ],
        'Type', required=True, index=True, change_default=True,default='directory',
    ...
    domain= fields.Char('Domain', size=128, help="Use a domain if you want to apply an automatic filter on visible resources.",default=[])
    ...
    resource_find_all=fields.Boolean('Find all resources',
            help="If true, all attachments that match this resource will " \
                " be located. If false, only ones that have this as parent.",default=True)
    ...
    ressource_id=fields.Integer('Resource ID',
        help="Along with Parent Model, this ID attaches this folder to a specific record of Parent Model.",default=0)
    ...
    company_id=fields.Many2one('res.company', 'Company', change_default=True,default=lambda s: s.env['res.company']._company_default_get('document.directory'))
    ...
    def get_full_path(self):
        ...
        """
        dir_id=self.ids
        if isinstance(self.ids, (tuple, list)):
            assert len(self.ids) == 1
            dir_id = self.ids[0]

        def _parent(dir_id, path):
            parent=self.browse(dir_id)
    ...
    def _check_recursion(self):
        ...
        ids=self.ids
        while len(ids):
            self._cr.execute('select distinct parent_id from document_directory where id in ('+','.join(map(str,ids))+')')
    ...
    def get_object(self,uri):
        ...
        """
        return get_node_context(self._cr,self._uid,self._context).get_uri(self._cr,uri)

    @api.model
    def get_node_class(self,dbro=None, dynamic=False):
    ...
    def get_dir_permissions(self):
        ...
        for pperms in [('read', 5), ('write', 2), ('unlink', 8)]:
            try:
                self.check_access_rule(pperms[0])
                res |= pperms[1]
            except except_orm:
                pass
        return res
    ...
    @api.model
    def _locate_child(self,uri, nparent, ncontext):
    ...
    def copy(self,default=None):
        if not default:
            default ={}
        name = self.read([self.id])[0]['name']
        default.update(name=_("%s (copy)") % (name))
        return super(document_directory,self).copy(default)
    ...
    def _check_duplication(self,vals, ids=None, op='create'):
        ...
        if op=='write':
            for directory in self.browse(ids):
                ...
                res=self.search([('id','<>',directory.id),('name','=',name),('parent_id','=',parent_id),('ressource_parent_type_id','=',ressource_parent_type_id),('ressource_id','=',ressource_id)])
                if len(res):
                    return False
        if op=='create':
            res = self.search([('name','=',name),('parent_id','=',parent_id),('ressource_parent_type_id','=',ressource_parent_type_id),('ressource_id','=',ressource_id)])
            if len(res):
                return False
        return True

    def write(self,vals):
        if not self._check_duplication(vals, self.ids, op='write'):
            raise ValidationError(_('Directory name must be unique!'))
        return super(document_directory,self).write(vals)

    def create(self,vals):
        if not self._check_duplication(vals):
            raise ValidationError(_('Directory name must be unique!'))
...
class document_directory_dctx(models.Model):
    ...
    field=fields.Char('Field', size=20, required=True, index=True, help="The name of the field.")
...
class document_directory_content_type(models.Model):
...
class document_directory_content(models.Model):
    ...
    def _extension_get(self):
        self._cr.execute('select code,name from document_directory_content_type where active')
    ...
    extension=fields.Selection(_extension_get, 'Document Type', required=True, size=4,default='.pdf')
    include_name=fields.Boolean('Include Record Name',
            help="Check this field if you want that the name of the file to contain the record name." \
                "\nIf set, the directory will have to be a resource one.",default=True)
    ...
    @api.model
    def process_write(self,node, data):
        if node.extension != '.pdf':
            raise Exception("Invalid content: %s" % node.extension)
        return True

    @api.model
    def process_read(self,node):
        if node.extension != '.pdf':
            raise Exception("Invalid content: %s" % node.extension)
        report = self.env['ir.actions.report.xml'].browse(node.report_id)
        srv=self.env['ir.actions.report.xml']._lookup_report('report.'+report.report_name)
        ctx = node.context.context.copy()
        ctx.update(node.dctx)
        cr, uid, context = self.env.args
        pdf = srv.create(cr, uid,[node.act_id], {}, context=ctx)
        return pdf


class ir_action_report_xml(models.Model):
    ...
    def _model_get(self):
        ...
        for data in self.read(['model']):
            model = data.get('model',False)
            if model:
                model_id =model_pool.search([('model','=',model)])
                if model_id:
                    res[data.get('id')] = model_id[0].id
                else:
                    res[data.get('id')] = False
        return res

    @api.multi
    def _model_search(self,args, offset=0, limit=None, order=None,count=False):
        if not len(args):
            return []
        ...
        if not model_id:
            ...
            return False
        model = self.env['ir.model'].read([model_id])[0]['model']
        report_id = [x['id'] for x in self.search_read(
            [('model', '=', model)], ['id'])]
        if not report_id:
            return [('id','=','0')]
        return [('id','in',report_id)]

    model_id=fields.Integer(compute='_model_get', search='_model_search', string='Model Id')


class document_storage(models.Model):
    """ The primary object for data storage. Deprecated.  """
    _name = 'document.storage'
    _description = 'Storage Media'

    @api.model
    def get_data(self,file_node,fil_obj=None):
        ...
        if fil_obj:
            ...
            ira = self.env['ir.attachment'].browse(file_node.file_id)
        data = ira.datas
        if data:
    ...
    @api.model
    def get_file(self,file_node, mode):
    ...
    @api.model
    def set_data(self,file_node, data,fil_obj=None):
        """ store the data.
        ...
        fname = None
        filesize = len(data)
        ira.write({'datas': data.encode('base64')})
        ...
        try:
            ...
            self._cr.execute('UPDATE ir_attachment SET file_size = %s, index_content = %s, file_type = %s WHERE id = %s', (filesize, icont_u, mime, file_node.file_id))
...
class node_context(object):
    ...
    def get_uri(self, cr, uri):
        ...
        """
        (ndir, duri) =  self._dirobj._locate_child(uri, None, self)
        while duri:
    ...
    def get_dir_node(self, cr, dbro):
        ...
        fullpath = dbro.get_full_path()
        klass = dbro.get_node_class(dbro)
...
class node_database(node_class):
    ...
    def _child_get(self, cr, name=False, domain=None):
        ...
        if domain:
            where = where + domain
        res = []
        for dirr in dirobj.search(where):
            klass = dirr.get_node_class(dirr)
...
class node_dir(node_database):
    ...
    def _file_get(self, cr, nodename=False):
        ...
        if not is_allowed:
            raise IOError(errno.EPERM, "Permission into directory denied.")
        env = api.Environment(cr, SUPERUSER_ID, {})
        cntobj = env['document.directory.content']
        uid = self.context.uid
        ctx = self.context.context.copy()
        ctx.update(self.dctx)
        where = [('directory_id','=',self.dir_id) ]
        for content in cntobj.search(where):
    ...
    def _child_get(self, cr, name=None, domain=None):
        ...
        res = []
        for dirr in dirobj.search(where2):
        ...
        env = api.Environment(cr, SUPERUSER_ID, {})
        fil_obj = env['ir.attachment']
        for fil in fil_obj.search(where):
    ...
    def rmcol(self, cr):
        uid = self.context.uid
        directory = self.context._dirobj.browse(self.dir_id)
        res = False
        if not directory:
    ...
    def create_child(self, cr, path, data=None):
        ...
        fil_id = fil_obj.create(val).id
        fil = fil_obj.browse(fil_id)
    ...
    def move_to(self, cr, ndir_node, new_name=False, fil_obj=None, ndir_obj=None, in_write=False):
        ...
        if not fil_obj:
            dbro = dir_obj.browse(self.dir_id)
        else:
            dbro = dir_obj
...
class node_res_dir(node_class):
    ...
    def _child_get(self, cr, name=None, domain=None):
        ...
        if self.ressource_tree:
            object2 = False
            if self.resm_id:
                object2 = obj.browse(self.resm_id) or False
            if obj._parent_name in obj.fields_get():
                where.append((obj._parent_name,'=',object2 and object2.id or False))

        res = []
        for bo in obj.search(where):
            if not bo:
                continue
...
class node_res_obj(node_class):
    ...
    def _child_get(self, cr, name=None, domain=None):
        ...
        ctx = self.context.context.copy()
        ctx.update(self.dctx)
        directory = dirobj.browse(self.dir_id)
        env = api.Environment(cr, SUPERUSER_ID, {})
        obj = env[self.res_model]
        where = []
        res = []
        ...
        for fil in fil_obj.search(where3):
            klass = self.context.node_file_class
            res.append(klass(fil.name, self, self.context, fil))
        ...
        if directory.ressource_type_id and directory.ressource_type_id.id:
            ...
            for dirr in dirobj.search(where5):
    ...
    def create_child_collection(self, cr, objname):
        ...
        ctx.update(self.dctx)
        env = api.Environment(cr, SUPERUSER_ID, {})
        res_obj = env[self.res_model]

        object2 = res_obj.browse(self.res_id) or False
...
class node_file(node_class):
    ...
    def open_data(self, cr, mode):
        ...
        env = api.Environment(cr, SUPERUSER_ID, {})
        stobj = env['document.storage']
        return stobj.get_file(self, mode=mode, context=self.context.context)
    ...
    def rm(self, cr):
        ...
        if not self.check_perms(8):
            raise IOError(errno.EPERM, "Permission denied.")
        env = api.Environment(cr, SUPERUSER_ID, {})
        document_obj = env['ir.attachment']
        if self.type in ('collection','database'):
            return False
        document = document_obj.browse(self.file_id)
        res = False
    ...
    def fix_ppath(self, cr, fbro):
        ...
        if fbro.parent_id:
            env = api.Environment(cr, SUPERUSER_ID, {})
            dirobj = env['document.directory']
            dirobj=dirobj.browse(fbro.parent_id.id)
            dirpath = dirobj.get_full_path()
        if fbro.datas_fname:
    ...
    def get_data(self, cr, fil_obj=None):
        ...
        env = api.Environment(cr, SUPERUSER_ID, {})
        stobj = env['document.storage']
        return stobj.get_data(self,fil_obj)
    ...
    def set_data(self, cr, data, fil_obj=None):
        ...
        env = api.Environment(cr, SUPERUSER_ID, {})
        stobj = env['document.storage']
        return stobj.set_data(self, data, fil_obj)
    ...
    def move_to(self, cr, ndir_node, new_name=False, fil_obj=None, ndir_obj=None, in_write=False):
        ...
        if (not self.check_perms(8)) and ndir_node.check_perms(2):
            raise IOError(errno.EPERM, "Permission denied.")
        env = api.Environment(cr, SUPERUSER_ID, {})
        doc_obj = env['ir.attachment']
        if not fil_obj:
            dbro = doc_obj.browse(self.file_id)
        else:
            dbro = fil_obj
        ...
        if ndir_node and self.parent != ndir_node:
            ...
            if not ndir_obj:
                ndir_obj = self.context._dirobj.browse(ndir_node.dir_id)
...
class node_content(node_class):
    ...
    def get_data(self, cr, fil_obj=None):
        ...
        ctx = self.context.context.copy()
        ctx.update(self.dctx)
        data = cntobj.process_read(self, ctx)
        if data:
    ...
    def set_data(self, cr, data, fil_obj=None):
        ...
        ctx = self.context.context.copy()
        ctx.update(self.dctx)
        return cntobj.process_write(self, data)
...
class nodefd_content(StringIO, node_descriptor):
    ...
    def __init__(self, parent, cr, mode, ctx):
        ...
        if mode in ('r', 'r+'):
            env = api.Environment(cr, SUPERUSER_ID, {})
            cntobj = env['document.directory.content']
            data = cntobj.process_read(parent)
            if data:
                self._size = len(data)
    ...
    def close(self):
        ...
        uid = par.context.uid
        registry = odoo.registry(par.context.dbname)
        cr = registry.cursor()
        try:
    ...
    def close(self):
        ...
        with registry.cursor() as cr:
            ...
            if self.mode in ('w', 'w+', 'r+'):
                env = api.Environment(cr, SUPERUSER_ID, {})
                fil_obj = env['ir.attachment']
                fil_obj=fil_obj.browse(par.file_id)
                fil_obj.write({'datas': data})
            cr.commit()
        StringIO.close(self)
