...
class select_interpreter_line(osv.osv):
    ...
    def onclick_select_interpreter(self, cr, uid, ids, context={}):
        curr_rec = self.browse(cr, uid, ids[0])
        values = {'select_interpreter': True} if not curr_rec.select_interpreter else {'select_interpreter': False}
        self.write(cr, uid, curr_rec.id, values)
        return True
    ...
    def leave_voicemail_without_warning(self, cr, uid, ids, context=None):
        ...
        if not cur_obj.interpreter_id.is_agency:
            history_ids2 = history_obj.search(cr, SUPERUSER_ID, [('name', '=', cur_obj.interpreter_id.id),('state', 'in', ('confirm', 'allocated')),
    ...
    def leave_voicemail(self , cr ,uid , ids , context= None):
        ...
        res = self.leave_voicemail_without_warning(cr, uid, ids, context=context)
        ir_model_data = self.pool.get('ir.model.data')
        warning_obj = self.pool.get('warning')
...
#            print "user_ids......",user_ids
    ...
    def job_offer_multiple_interpreters(self, cr, uid, ids, context=None):
