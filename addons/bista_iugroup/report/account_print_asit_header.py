...
class account_invoice(report_sxw.rml_parse):
    ...
    def line_cal(self,model_id):
        ...
        for i in model_id.invoice_line_ids:
            vals={'quantity':i.quantity,'price_unit':i.price_unit,'mileage':i.mileage,'mileage_rate':i.mileage_rate,'discount':i.discount,'travel_time':i.travel_time,'travelling_rate':i.travelling_rate}
            res.append(vals)
        return res
...
#            travel_sum.append(round(each_line['travel_time']*each_line['travelling_rate'],2))
	    travel_sum.append(each_line['travel_time']*each_line['travelling_rate'])
...
#        vals=self.line_cal(model_id)
#        result, min, hr = False, False, False
#        qty = vals[0]['quantity']
#        if not qty:
#            qty = '00'
#            result = '00 Hours 00 Minutes'
#        else:
#            min = math.floor((qty - math.floor(qty))*100)
#            hr = math.floor(qty)
#            result = '%d Hours %d Minutes'%(hr,min)
#        return result
...
#            base = str(round(base))+'0' if base else False
#            index =  base.find('.') if base else False
#            if index >= 0:
#                base = base[:index+3] if len(base[index+1 :]) > 2 else base
#
#            if not base:
#                base=0.00
#            mileage=str(model_id.invoice_line.mileage[0]*model_id.invoice_line.mileage_rate[0])
#            mileage = str(round(mileage))+'0' if mileage else False
#            index1 =  mileage.find('.') if mileage else False
#            if index1 >= 0:
#                mileage = mileage[:index1 + 3] if len(mileage[index1 + 1 :]) > 2 else mileage
#
#            if not mileage:
#                mileage=0.00
...
#                    res['medical_file_no']=claims_val.get('claim_number')
#                elif claims_val.get('claim_no'):
#                    res['medical_file_no']=claims_val.get('claim_no')
#                elif claims_val.get('claim_no2'):
#                    res['medical_file_no']=claims_val.get('claim_no2')
#                else:
#                    res['medical_file_no']= ''
        ...
        try:
            if model_id.event_id:
                a=''
                for lang in model_id.event_id.language_id2:
                    if lang:
                        a=a+" "+lang.name
                res['language2'] = a
            else:
        ...
        try:
            ...
            res['service']=''
#        try:
#            location='Location: '+(model_id.location_id.name if model_id.location_id and model_id.location_id.name else '')+', '+'\n              '\
#                        +(model_id.location_id.street if model_id.location_id and model_id.location_id.street else '')+', '+'\n              '\
#                        +(model_id.location_id.city if model_id.location_id and model_id.location_id.city else '')+', '\
#                        +(model_id.location_id.state_id.name if model_id.location_id and model_id.location_id.state_id and model_id.location_id.state_id.name else '')+', '+'\n              '\
#                        +(model_id.location_id.zip if model_id.location_id and model_id.location_id.zip else '')
#
#        except Exception as e:
#            location='Location: '
        try:
            if model_id.event_id.ordering_contact_id:
                res['requested_by']=str(model_id.event_id.ordering_contact_id.name)+' '+str(model_id.event_id.ordering_contact_id.last_name)
            elif model_id.event_id.ordering_partner_id:
            ...
            res['requested_by']=''
        try:
            department = model_id.event_id and model_id.event_id.department or ''
            if department:
            ...
            res['department'] = ''
        try:
            if model_id.event_id and model_id.event_id.project_name_id:
                res['project_name']= model_id.event_id.project_name_id.name or ''
            else:
                res['project_name']= ''
        except Exception as e:
            res['project_name']= ''
...
#        address=textwrap.fill(complete_address,width=80)
        return add
report_sxw.report_sxw( 'report.account.invoice.asit.header', 'account.invoice',
    'custom_addons/bista_iugroup/report/account_print_asit_header.rml', parser=account_invoice )
