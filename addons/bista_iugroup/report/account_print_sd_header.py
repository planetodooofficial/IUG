...
#    _name = 'account.invoice'
    def __init__(self, cr, uid, name, context):
        ...
        self.localcontext.update({
            ...
            'print_mileage_rate': self.print_mileage_rate,
            'notes': self.notes,
            ...
            'base': self.base,
            'event_type': self.print_event_type,
    ...
    def base(self, model_id):
        ...
        for line in model_id.invoice_line_ids:
            _logger.info('++++++++++++++++base_sum++++++++++++%s',model_id)
    ...
    #
    #
    # def print_mileage_rate(self, model_id):
    #     mileage_rate = '00'
    #     vals = self.line_cal(model_id)
    #     if vals:
    #         mileage_rate = vals[0]['mileage_rate']
    #     return mileage_rate
    ...
    def print_mileage(self, model_id):
        ...
        if vals:
            for rec in vals:
                _logger.info(">>>>>>>>>>>>>>>>>>>>>>>print mileage vals>>>>%s>>>>", rec.get('mileage'))
                if rec.get('mileage'):
                    mileage += rec.get('mileage')
                    _logger.info(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>Mileage>>>>%s>>>>", mileage)
        return mileage

    def print_event_type(self, model_id):

        if model_id:
            event_type=model_id.event_type
            return event_type
    ...
    def print_mileage_rate(self, model_id):
        ...
        if vals:
            ...
            for rec in vals:
                if rec.get('mileage_rate') and rec.get('mileage_rate') > mileage_rate:
                    mileage_rate = rec.get('mileage_rate')
                    _logger.info(">>>>>>>>>>>>mileage_rate>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>%s>>>", mileage_rate)
...
#                    res['medical_file_no']= ''
        ...
        try:
            if model_id.event_id:
                ...
                res['language2'] = ''
                # a=model_id.event_id and model_id.event_id.language_id2.name or ''
        except Exception as e:
            res['language2'] = ''
        try:
            if model_id.event_id and model_id.event_id.multi_type:
                if model_id.event_id.multi_type != '1':
                    res['interpreter_type'] = model_id.event_id.multi_type or ''
                else:
                    res['interpreter_type']= ''
        except Exception as e:
            res['interpreter_type'] = ''
        try:
        ...
        try:
            ...
            res['department'] = ''
        try:
            strs =  model_id.event_id and model_id.event_id.project_name_id and model_id.event_id.project_name_id.name or ''
            p_name = ''
            for x in strs.splitlines():
                p_name = '\n'.join(line.strip() for line in re.findall(r'.{1,57}(?:\s+|$)', x))

            # p_name = re.sub("(.{57})", "\\1\n", project_name, 0, re.DOTALL)
            res['project_name']=p_name or ''
...
#            customer_comment = tag_re.sub(' ', str(o.partner_id.rubrik))
            ...
            customer_comment = customer_comment[0:500]
...
report_sxw.report_sxw( 'report.account.invoice.sd.header', 'account.invoice',
    'custom_addons/bista_iugroup/report/account_print_sd_header.rml', parser=account_invoice )
