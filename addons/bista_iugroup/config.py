...
class interpreter_language(models.Model):
    ...
    certification_exp_date=fields.Date("Expiry Date")

    @api.multi
    def interpreter_certification_reminder(self):
        context={}
        ir_model_data = self.env['ir.model.data']
        today_date= datetime.datetime.now().strftime('%Y-%m-%d')
        expiry_date = (datetime.datetime.now() + relativedelta.relativedelta(days=13)).strftime('%Y-%m-%d')

        interpt_lang = self.search(
            [('certification_exp_date', '<=', expiry_date), ('certification_level_id.name', '!=', 'Uncertified')])
        _logger.info(">>>>>>>>>>>>>interpt_langinterpt_langinterpt_lang>>>>>>>>>%s>>>>", interpt_lang)
        inter_template_id = ir_model_data.get_object_reference('bista_iugroup', 'email_template_interpreter_certification_reminder')[1]
        staff_template_id = ir_model_data.get_object_reference('bista_iugroup', 'email_template_staff_certification_reminder')[1]
        for language in interpt_lang:
            send_interpreter_email = self.env['mail.template'].browse(inter_template_id).send_mail(language.id, force_send=True)
            _logger.info(">>>>>>>>>>>>>>>>>>> send_interprete>>>>>%s>>>", send_interpreter_email)
        context['interpreter'] = interpt_lang
        _logger.info(">>>>>>>>>>>>>context['interpreter']>>>>>>>>>%s>>>>",context)
        send_staff_email = self.env['mail.template'].browse(staff_template_id).with_context(interpreter=interpt_lang).send_mail(language.id, force_send=True)
        _logger.info(">>>>>>>>>>>>>>>>>>> send_staff_email>>>>>%s>>>", send_staff_email)
        return True
...
class vaccines(models.Model):
    ...
    @api.multi
    def vaccines_certification_reminder(self):
        context = {}
        ir_model_data = self.env['ir.model.data']
        today_date = datetime.datetime.now().strftime('%Y-%m-%d')
        expiry_date = (datetime.datetime.now() + relativedelta.relativedelta(days=13)).strftime('%Y-%m-%d')

        vaccines = self.search(
            [('certification_exp_date', '<=', expiry_date)])
        _logger.info(">>>>>>>>>>>>>vaccines>>>>>>>>>%s>>>>", vaccines)
        inter_template_id = ir_model_data.get_object_reference('bista_iugroup', 'email_template_interpreter_certification_reminder_vac')[1]
        staff_template_id = ir_model_data.get_object_reference('bista_iugroup', 'email_template_staff_certification_reminder_vac')[1]
        for vac in vaccines:
            send_interpreter_email = self.env['mail.template'].browse(inter_template_id).send_mail(vac.id,
                                                                                                   force_send=True)
            _logger.info(">>>>>>>>>>>>>>>>>>> send_interprete>>>>>%s>>>", send_interpreter_email)
        context['vaccines'] = vaccines
        _logger.info(">>>>>>>>>>>>>context['vaccines']>>>>>>>>>%s>>>>", context)
        send_staff_email = self.env['mail.template'].browse(staff_template_id).with_context(vaccines=vaccines).send_mail(vac.id, force_send=True)
        _logger.info(">>>>>>>>>>>>>>>>>>> send_staff_email>>>>>%s>>>", send_staff_email)
        return True
...
class rate(models.Model):
    ...
    rate_type=fields.Selection([('normal','Legal'),('medical','Medical'),('deposition','Depositions'),('conf_call','Conf Call'),('travel','Travel'),('other','Other')],"Rate Type",
                                  required=True)
    day_type=fields.Selection([('weekday','Weekday'),('even_night','Evening/Overnight'),('weekend','Weekend')],"Day Type", default='weekday', required=True)
    rush_rate=fields.Float("Rush")
    start_time=fields.Char("start Time", size=2, default='00')
    am_pm1 = fields.Selection([('am', 'AM'), ('pm', 'PM')], "AM/PM", default='am')
    end_time = fields.Char("End Time", size=2, default='00')
    am_pm2 = fields.Selection([('am', 'AM'), ('pm', 'PM')], "AM/PM", default='pm')
    ...
    def _check_base_hr_min(self):
        for rate in self:
            if rate.rate_type not in ('conf_call','medical','other') and (rate.base_hour == '1min' or rate.inc_min == '1min'):
