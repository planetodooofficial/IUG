...
class account_invoice_xls_parser_hipaa(report_sxw.rml_parse):

    def __init__(self, cr, uid, name, context):
        super(account_invoice_xls_parser_hipaa, self).__init__(cr, uid, name, context=context)
        self.env = Environment(cr, uid, context)
        invoice_line_obj = self.env['account.invoice.line']
        self.context = context
        wanted_list = invoice_line_obj._report_xls_fields_hipaa()
        template_changes = invoice_line_obj._report_xls_template_hipaa()
        space_extra = invoice_line_obj._report_xls_render_space_extra_hipaa()
...
class account_invoice_xls_hipaa(report_xls):
    ''' Hipaa report '''
    ...
    def __init__(self, name, table, rml=False, parser=False, header=True, store=False):
        super(account_invoice_xls_hipaa, self).__init__(name, table, rml, parser, header, store)
        ...
        self.col_specs_template_hipaa = {
            ...
            'date_service': {
                ...
                'lines': [1, 0, 'text', _render("line.invoice_id.event_id and (line.invoice_id.event_id.event_start_date) and datetime.strftime(datetime.strptime(line.invoice_id.event_id.event_start_date,'%Y-%m-%d'),'%m-%d-%Y') or '' ")],
                'totals': [1, 0, 'text', None]},
            'language': {
            ...
            'patient_name': {
                ...
                'lines': [1, 0, 'text', self.parse_unicode(_render("(line.invoice_id.event_id and line.invoice_id.event_id.patient_id and line.invoice_id.event_id.patient_id.last_name or '') +' '+(line.invoice_id.event_id and line.invoice_id.event_id.patient_id and line.invoice_id.event_id.patient_id.name and line.invoice_id.event_id.patient_id.name[0] or '')"))],
                'totals': [1, 0, 'text', None]},
            'requester': {
...
#                                                _render("line.invoice_id.event_id and line.invoice_id.event_id.location_id and line.invoice_id.event_id.location_id.street2 or '' "))],
            ...
            'date_invoice': {
                ...
                'lines': [1, 0, 'text', _render("line.invoice_id.date_invoice and datetime.strftime(datetime.strptime(line.invoice_id.date_invoice,'%Y-%m-%d'),'%m-%d-%Y') or '' or ''")],
                'totals': [1, 0, 'text', None]},
            'amount': {
            ...
            'event_ref': {
                ...
                'lines': [1, 0, 'text', _render("line.invoice_id.event_id and str(line.invoice_id.event_id.ref or '') or ''")],
                'totals': [1, 0, 'text', None]},
            'event_project_id': {
                'header': [1, 13, 'text', _render("_('Event Project')")],
                'lines': [1, 0, 'text', _render("line.invoice_id.event_id and str(line.invoice_id.event_id.project_name_id.name or '') or ''")],
                'totals': [1, 0, 'text', None]},                           
    ...
    def generate_xls_report(self, _p, _xs, data, objects, wb):
        ...
        self.col_specs_template_hipaa.update(_p.template_changes)
...
#        row_pos += 1
        # Column headers
        c_specs = map(lambda x: self.render(x, self.col_specs_template_hipaa, 'header', render_space={'_': _p._}), wanted_list)
...
#            cnt += 1
            ...
            c_specs = map(lambda x: self.render(x, self.col_specs_template_hipaa, 'lines'), wanted_list)
            row_data = self.xls_row_template(c_specs, [x[0] for x in c_specs])
        ...
        amount_cell = rowcol_to_cell(row_pos, amount_pos)
        c_specs = map(lambda x: self.render(x, self.col_specs_template_hipaa, 'totals'), wanted_list)
        row_data = self.xls_row_template(c_specs, [x[0] for x in c_specs])
        row_pos = self.xls_write_row(ws, row_pos, row_data, row_style=self.rt_cell_style_right)
        return True

account_invoice_xls_hipaa('report.account.invoice.xls.print.hipaa', 'account.invoice.line',
    parser=account_invoice_xls_parser_hipaa)
