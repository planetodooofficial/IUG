...
class account_invoice_xls_parser_john_muir(report_sxw.rml_parse):

    def __init__(self, cr, uid, name, context):
        super(account_invoice_xls_parser_john_muir, self).__init__(cr, uid, name, context=context)
        self.env = Environment(cr, uid, context)
        invoice_line_obj = self.env['account.invoice.line']
        self.context = context
        wanted_list = invoice_line_obj._report_xls_fields_john_muir()
        template_changes = invoice_line_obj._report_xls_template_john_muir()
        space_extra = invoice_line_obj._report_xls_render_space_extra_john_muir()
...
class account_invoice_xls_john_muir(report_xls):
    ...
    def __init__(self, name, table, rml=False, parser=False, header=True, store=False):
        super(account_invoice_xls_john_muir, self).__init__(name, table, rml, parser, header, store)
        # Cell Styles
        _xs = self.xls_styles
        # header
        ...
        self.col_specs_template_john_muir = {
            'id': {
            ...
            'ordering_contact_phone': {
                ...
                'lines': [1, 0, 'text', self.parse_unicode(_render("line.invoice_id.event_id and line.invoice_id.event_id.ordering_contact_id and line.invoice_id.event_id.ordering_contact_id.phone or line.invoice_id.event_id.ordering_contact_id.phone2 or '' "))],
                'totals': [1, 0, 'text', None]},
            'billing_customer': {
...
#                                                _render("line.invoice_id.event_id and line.invoice_id.event_id.location_id and line.invoice_id.event_id.location_id.street2 or '' "))],
            ...
            'rate': {
                ...
                'lines': [1, 0, 'number', _render("supp_inv_rate and round(supp_inv_rate) or 0 ")],
                'totals': [1, 0, 'text', None]},
            'miles_driven': {
            ...
            'cost_center': {
                ...
                'lines': [1, 0, 'text', self.parse_unicode(_render("line.invoice_id.event_id and line.invoice_id.event_id.cost_center or ''"))],
                'totals': [1, 0, 'text', None]},
            'status': {
            ...
            'patient_medical_number': {
                ...
                'totals': [1, 0, 'text', None]},
            'event_outcome': {
                'header': [1, 20, 'text', _render("_('Event Outcome')")],
    ...
    def generate_xls_report(self, _p, _xs, data, objects, wb):
        ...
        self.col_specs_template_john_muir.update(_p.template_changes)
...
#        row_pos += 1
        # Column headers
        c_specs = map(lambda x: self.render(x, self.col_specs_template_john_muir, 'header', render_space={'_': _p._}), wanted_list)
...
#        cnt = 0
        for line in objects:
            invoice_obj_srch = self.env['account.invoice'].search([('type','=','in_invoice'),('event_id','=',line.invoice_id.event_id.id)])
            for each_supp_rec in invoice_obj_srch:
                supp_inv_rate = each_supp_rec.invoice_line_ids and each_supp_rec.invoice_line_ids[0].price_unit or 0.0
#            cnt += 1
            amount_cell = rowcol_to_cell(row_pos, amount_pos)
            c_specs = map(lambda x: self.render(x, self.col_specs_template_john_muir, 'lines'), wanted_list)
            row_data = self.xls_row_template(c_specs, [x[0] for x in c_specs])
        ...
        amount_cell = rowcol_to_cell(row_pos, amount_pos)
        c_specs = map(lambda x: self.render(x, self.col_specs_template_john_muir, 'totals'), wanted_list)
        row_data = self.xls_row_template(c_specs, [x[0] for x in c_specs])
        row_pos = self.xls_write_row(ws, row_pos, row_data, row_style=self.rt_cell_style_right)
        return True

account_invoice_xls_john_muir('report.account.invoice.xls.print.john.muir', 'account.invoice.line',
    parser=account_invoice_xls_parser_john_muir)
