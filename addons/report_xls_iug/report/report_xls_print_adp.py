...
class account_invoice_xls_parser_adp(report_sxw.rml_parse):

    def __init__(self, cr, uid, name, context):
        super(account_invoice_xls_parser_adp, self).__init__(cr, uid, name, context=context)
        self.env = Environment(cr, uid, context)
        invoice_line_obj = self.env['account.invoice.line']
        self.context = context
        wanted_list = invoice_line_obj._report_xls_fields_adp()
...
class account_invoice_xls_adp(report_xls):
    ...
    def __init__(self, name, table, rml=False, parser=False, header=True, store=False):
        super(account_invoice_xls_adp, self).__init__(name, table, rml, parser, header, store)
...
#                                                _render("line.invoice_id.event_id and line.invoice_id.event_id.location_id and line.invoice_id.event_id.location_id.street2 or '' "))],
    ...
    def generate_xls_report(self, _p, _xs, data, objects, wb):
        ...
        self.col_specs_template_adp.update(_p.template_changes)
...
#        row_pos += 1
        # Column headers
        c_specs = map(lambda x: self.render(x, self.col_specs_template_adp, 'header', render_space={'_': _p._}), wanted_list)
...
#            cnt += 1
            ...
            c_specs = map(lambda x: self.render(x, self.col_specs_template_adp, 'lines'), wanted_list)
            row_data = self.xls_row_template(c_specs, [x[0] for x in c_specs])
        ...
        amount_cell = rowcol_to_cell(row_pos, amount_pos)
        c_specs = map(lambda x: self.render(x, self.col_specs_template_adp, 'totals'), wanted_list)
        row_data = self.xls_row_template(c_specs, [x[0] for x in c_specs])
        row_pos = self.xls_write_row(ws, row_pos, row_data, row_style=self.rt_cell_style_right)
        return True

account_invoice_xls_adp('report.account.invoice.xls.print.adp', 'account.invoice.line',
    parser=account_invoice_xls_parser_adp)
