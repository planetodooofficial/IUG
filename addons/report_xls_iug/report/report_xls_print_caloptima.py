...
class account_invoice_xls_parser_caloptima(report_sxw.rml_parse):

    def __init__(self, cr, uid, name, context):
        super(account_invoice_xls_parser_caloptima, self).__init__(cr, uid, name, context=context)
        self.env = Environment(cr, uid, context)
        invoice_line_obj = self.env['account.invoice.line']
        self.context = context
        wanted_list = invoice_line_obj._report_xls_fields_caloptima()
        template_changes = invoice_line_obj._report_xls_template_caloptima()
        space_extra = invoice_line_obj._report_xls_render_space_extra_caloptima()
...
class account_invoice_xls_caloptima(report_xls):
    ...
    def __init__(self, name, table, rml=False, parser=False, header=True, store=False):
        super(account_invoice_xls_caloptima, self).__init__(name, table, rml, parser, header, store)
        ...
        self.col_specs_template_caloptima = {
            'id': {
...
#                                                _render("line.invoice_id.event_id and line.invoice_id.event_id.location_id and line.invoice_id.event_id.location_id.street2 or '' "))],
            ...
            'duration': {
                ...
                'lines': [1, 0, 'text', _render("line.task_line_id and str(round((line.task_line_id.time_spent or ''), 2))or ''")],
                'totals': [1, 0, 'text', None]},
            'rate': {
            ...
            'patient_medical_number': {
                ...
                'totals': [1, 0, 'text', None]},
            'job_note': {
                'header': [1, 40, 'text', _render("_('Job Note')")],
                'lines': [1, 0, 'text', self.parse_unicode(
                    _render("line.invoice_id.event_id and line.invoice_id.event_id.comment or '' "))],
                'totals': [1, 0, 'text', None]},
        }
    ...
    def generate_xls_report(self, _p, _xs, data, objects, wb):
        ...
        self.col_specs_template_caloptima.update(_p.template_changes)
	#_logger.warn("test data %s,%s,%s", wanted_list,data,objects,wb)
...
#        row_pos += 1
        # Column headers
        c_specs = map(lambda x: self.render(x, self.col_specs_template_caloptima, 'header', render_space={'_': _p._}), wanted_list)
...
#            cnt += 1
            ...
            c_specs = map(lambda x: self.render(x, self.col_specs_template_caloptima, 'lines'), wanted_list)
            try:
                if c_specs and c_specs[34] and c_specs[34][4]:
                    c_specs[34][4]=jQ(c_specs[34][4]).text()
            except:
                pass
            row_data = self.xls_row_template(c_specs, [x[0] for x in c_specs])
        ...
        amount_cell = rowcol_to_cell(row_pos, amount_pos)
        c_specs = map(lambda x: self.render(x, self.col_specs_template_caloptima, 'totals'), wanted_list)
        row_data = self.xls_row_template(c_specs, [x[0] for x in c_specs])
        row_pos = self.xls_write_row(ws, row_pos, row_data, row_style=self.rt_cell_style_right)
        return True

account_invoice_xls_caloptima('report.account.invoice.xls.print.caloptima', 'account.invoice.line',
    parser=account_invoice_xls_parser_caloptima)
