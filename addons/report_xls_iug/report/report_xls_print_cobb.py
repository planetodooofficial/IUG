...
class account_invoice_xls_parser_cobb(report_sxw.rml_parse):

    def __init__(self, cr, uid, name, context):
        super(account_invoice_xls_parser_cobb, self).__init__(cr, uid, name, context=context)
        self.env = Environment(cr, uid, context)
        invoice_line_obj = self.env['account.invoice.line']
        self.context = context
        wanted_list = invoice_line_obj._report_xls_fields_cobb()
...
class account_invoice_xls_cobb(report_xls):
    ...
    def parse_address(self, name='', street='', street2=''):
        address = ''
        print "name...street..street2.......",name,street,street2
        try:
            if name:
                name = name.encode('utf-8', 'ignore')
                try:
                    name = unicode(name, "ascii",'ignore')
                except UnicodeError:
                    name = unicode(name, "utf-8", 'ignore').decode('ascii')
            else:
                name = ''
            if street:
                ...
                street = ''
            if street2:
                street2 = street2.encode('utf-8', 'ignore')
                try:
                    street2 = unicode(street2, "ascii",'ignore')
                except UnicodeError:
                    street2 = unicode(street2, "utf-8", 'ignore').decode('ascii')
            else:
                street2 = ''
        except Exception:
            pass
    ...
    def __init__(self, name, table, rml=False, parser=False, header=True, store=False):
        super(account_invoice_xls_cobb, self).__init__(name, table, rml, parser, header, store)
        ...
        self.col_specs_template_cobb = {
            ...
            'zip': {
                ...
                'lines': [1, 0, 'text', self.parse_unicode(_render("line.invoice_id.partner_id.zip or '' "))],
                'totals': [1, 0, 'text', None]},
            'location': {
            ...
            'invoice_number': {
                ...
                'totals': [1, 0, 'text', None]},
            'old_invoice_number': {
                'header': [1, 13, 'text', _render("_('Old Invoice Number')")],
                'lines': [1, 0, 'text', self.parse_unicode(_render("str(line.invoice_id.invoice_old_number) or ''"))],
                'totals': [1, 0, 'text', None]},
            'office': {
            ...
            'patient_medical_number': {
                ...
                'totals': [1, 0, 'text', None]},
            'project': {
                'header': [1, 13, 'text', _render("_('Project')")],
                'lines': [1, 0, 'text', self.parse_unicode(_render(
                    "line.invoice_id.event_id and line.invoice_id.event_id.project_name_id.name or ''"))],
                'totals': [1, 0, 'text', None]},
            'ref': {
                'header': [1, 13, 'text', _render("_('Reference')")],
                'lines': [1, 0, 'text', self.parse_unicode(_render(
                    "line.invoice_id.event_id and line.invoice_id.event_id.ref or ''"))],
                'totals': [1, 0, 'text', None]},
            'event_department': {
                'header': [1, 13, 'text', _render("_('Suite/Dept/Social Worker')")],
                'lines': [1, 0, 'text', self.parse_unicode(_render(
    ...
    def generate_xls_report(self, _p, _xs, data, objects, wb):
        ...
        self.col_specs_template_cobb.update(_p.template_changes)
...
#        row_pos += 1
        # Column headers
        c_specs = map(lambda x: self.render(x, self.col_specs_template_cobb, 'header', render_space={'_': _p._}), wanted_list)
...
#            cnt += 1
            ...
            c_specs = map(lambda x: self.render(x, self.col_specs_template_cobb, 'lines'), wanted_list)
            row_data = self.xls_row_template(c_specs, [x[0] for x in c_specs])
        ...
        amount_cell = rowcol_to_cell(row_pos, amount_pos)
        c_specs = map(lambda x: self.render(x, self.col_specs_template_cobb, 'totals'), wanted_list)
        row_data = self.xls_row_template(c_specs, [x[0] for x in c_specs])
        row_pos = self.xls_write_row(ws, row_pos, row_data, row_style=self.rt_cell_style_right)
        return True

account_invoice_xls_cobb('report.account.invoice.xls.print.cobb', 'account.invoice.line',
    parser=account_invoice_xls_parser_cobb)
