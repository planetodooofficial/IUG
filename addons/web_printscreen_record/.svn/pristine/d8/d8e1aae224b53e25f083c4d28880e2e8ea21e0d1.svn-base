...
class RecordExcelExport(ExcelExport):
    ...
    @http.route(['/web/export/record_excel_export'], type='http', auth="user")
    @serialize_exception
    def index(self, data, token):
        data = json.loads(data)
        if data.get('model',False):
            filename=data['model']+'.xls'
        else:
            filename='Export.xls'
...
class ExportPdf(Export):
    _cp_path = '/web/export/record_pdf'
    fmt = {
        'tag': 'pdf',
        'label': 'PDF',
        'error': None
...
class RecordPdfExport(ExportPdf):


    @http.route(['/web/export/record_pdf_export'], type='http', auth="user")
    def index(self, req, data, token):
        data = json.loads(data)
        uid = data.get('uid', False)
        if data.get('model',False):
            filename=data['model']+'.pdf'
        else:
            filename='Export.pdf'
